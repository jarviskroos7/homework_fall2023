...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            # swapping row dimensions
            next_qs = next_qs[[1, 0], :]
        elif self.target_critic_backup_type == "min":
            # return the min prediction of the two critics
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # https://pytorch.org/docs/stable/distributions.html#torch.distributions.distribution.Distribution.rsample
        return -1 * torch.mean(action_distribution.log_prob(
            action_distribution.rsample(sample_shape=(self.num_actor_samples,))
        ), dim=0)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs = obs[None].expand((
                self.num_actor_samples, batch_size, *self.observation_shape
            ))
            q_values = self.critic(obs, action)
        ...
        # reused from my hw2
        loss = -1 * torch.mean(
            torch.mul(log_probs, advantage)
        ) # minimize negative loss -> maximize log likelihood
